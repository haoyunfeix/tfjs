...
describeWithFlags(
    ...
    {
        // predicate: testEnv => testEnv.backendName === 'webgpu' &&
        //    tfc.findBackend('webgpu') !== null && tfc.findBackend('cpu') !==
        //    null
    },
    ...
    () => {
      ...
      beforeAll(() => {
        webglCpuForwardFlagSaved = tfc.env().getBool('WEBGPU_CPU_FORWARD');
        tfc.env().set('WEBGPU_CPU_FORWARD', true);
      });

      afterAll(() => {
        tfc.env().set('WEBGPU_CPU_FORWARD', webglCpuForwardFlagSaved);
      });
